...
contains
  ...
  subroutine file_input(nd,mpirank,ro,pr,vx,vy,vz,bx,by,bz,phi,eta,ix,jx)

    integer,intent(in) :: nd,mpirank,ix,jx
    real(8),intent(inout),dimension(ix,jx) :: ro,pr,vx,vy,vz,bx,by,bz,phi,eta
    ...
    mfi_bx=75
    call dacopnr2s(mfi_bx,input_dir//cnond//'_bx_rank='//cno//'.dac',mt,ix0,jx0,nx0)
    read(mfi_bx) bx
    close(mfi_bx)

    mfi_by=76
    call dacopnr2s(mfi_by,input_dir//cnond//'_by_rank='//cno//'.dac',mt,ix0,jx0,nx0)
    read(mfi_by) by
    close(mfi_by)

    mfi_bz=77
    call dacopnr2s(mfi_bz,input_dir//cnond//'_bz_rank='//cno//'.dac',mt,ix0,jx0,nx0)
    read(mfi_bz) bz
    close(mfi_bz)

    mfi_phi=78
    call dacopnr2s(mfi_phi,input_dir//cnond//'_phi_rank='//cno//'.dac',mt,ix0,jx0,nx0)
    read(mfi_phi) phi
    close(mfi_phi)

    mfi_eta=79
    call dacopnr2s(mfi_eta,input_dir//cnond//'_eta_rank='//cno//'.dac',mt,ix0,jx0,nx0)
    read(mfi_eta) eta
    close(mfi_eta)
  ...
  subroutine file_output(nd,mpirank,ro,pr,vx,vy,vz,bx,by,bz,phi,eta,ix,jx)

    integer,intent(in) :: nd,mpirank,ix,jx
    real(8),intent(in),dimension(ix,jx) :: ro,pr,vx,vy,vz,bx,by,bz,phi,eta
    ...
    mf_bx=25
    call dacdef2s(mf_bx,output_dir//cnond//'_bx_rank='//cno//'.dac',6,ix,jx)
    write(mf_bx) bx
    close(mf_bx)

    mf_by=26
    call dacdef2s(mf_by,output_dir//cnond//'_by_rank='//cno//'.dac',6,ix,jx)
    write(mf_by) by
    close(mf_by)

    mf_bz=27
    call dacdef2s(mf_bz,output_dir//cnond//'_bz_rank='//cno//'.dac',6,ix,jx)
    write(mf_bz) bz
    close(mf_bz)
    
    mf_phi=28
    call dacdef2s(mf_phi,output_dir//cnond//'_phi_rank='//cno//'.dac',6,ix,jx)
    write(mf_phi) phi
    close(mf_phi)

    mf_eta=29
    call dacdef2s(mf_eta,output_dir//cnond//'_eta_rank='//cno//'.dac',6,ix,jx)
    write(mf_eta) eta
    close(mf_eta)
  ...
  subroutine file_output_param(nd,dtout,tend,ix,jx,igx,jgx,margin,mpisize &
    ...
    call dacputparamc(mf_params,'comment','model_OTvortex')
