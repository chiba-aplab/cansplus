...
contains
  ...
  subroutine flux_calc__hll(row,prw,vxw,vyw,vzw,bx,byw,bzw,gm,margin,ix,jx &
                           ,fro,fee,frx,fry,frz,fby,fbz)

  integer,intent(in) :: ix,jx,margin
  real(8),intent(in) :: gm
! primitive variables :: 1 = left state , 2 = right state
  real(8),dimension(ix,jx,2),intent(in) :: row,prw,vxw,vyw,vzw
  real(8),dimension(ix,jx,2),intent(in) :: byw,bzw
  real(8),dimension(ix,jx),intent(in) :: bx ! magnetic field 
  real(8),dimension(ix,jx),intent(out) :: fro,fee,frx,fry,frz ! numerical flux
  real(8),dimension(ix,jx),intent(out) :: fby,fbz
...
! compute HLL flux
!
        fro(i,j) = (sr*frol-sl*fror+sr*sl*(ror-rol))/(sr-sl)
        fee(i,j) = (sr*feel-sl*feer+sr*sl*(eer-eel))/(sr-sl)
           
        frx(i,j) = (sr*frxl-sl*frxr+sr*sl*(rxr-rxl))/(sr-sl)
        fry(i,j) = (sr*fryl-sl*fryr+sr*sl*(ryr-ryl))/(sr-sl)
        frz(i,j) = (sr*frzl-sl*frzr+sr*sl*(rzr-rzl))/(sr-sl)
           
        fby(i,j) = (sr*fbyl-sl*fbyr+sr*sl*(byr-byl))/(sr-sl)
        fbz(i,j) = (sr*fbzl-sl*fbzr+sr*sl*(bzr-bzl))/(sr-sl)
     end do
  end do
  !$OMP END PARALLEL DO
  ...
  subroutine flux_calc__hlld(row,prw,vxw,vyw,vzw,bx,byw,bzw,gm,margin,ix,jx &
                            ,fro,fee,frx,fry,frz,fby,fbz)
!=====================================================================
!
! Purpose
...
!            :: primitive value array(ix,jx,2)
!               qqw(i,j,1) = qq(i,j)'s left state
!               qqw(i,j,2) = qq(i,j)'s right state
...
!=====================================================================
  ...
  integer,intent(in) :: ix,jx,margin ! array size
...
! compute flux
        ...
        fro(i,j) = (frol-msl*rol-rolst*temp+roldst*mslst)*maxs1 &
                    +(fror-msr*ror-rorst*temp1+rordst*msrst)*mins1
        frx(i,j) = (frxl-msl*rxl-rxlst*temp+rxldst*mslst)*maxs1 &
                    +(frxr-msr*rxr-rxrst*temp1+rxrdst*msrst)*mins1
        fry(i,j) = (fryl-msl*ryl-rylst*temp+ryldst*mslst)*maxs1 &
                    +(fryr-msr*ryr-ryrst*temp1+ryrdst*msrst)*mins1
        frz(i,j) = (frzl-msl*rzl-rzlst*temp+rzldst*mslst)*maxs1 &
                    +(frzr-msr*rzr-rzrst*temp1+rzrdst*msrst)*mins1
        fee(i,j) = (feel-msl*eel-eelst*temp+eeldst*mslst)*maxs1 &
                    +(feer-msr*eer-eerst*temp1+eerdst*msrst)*mins1
        fby(i,j) = (fbyl-msl*byl-bylst*temp+byldst*mslst)*maxs1 &
                    +(fbyr-msr*byr-byrst*temp1+byrdst*msrst)*mins1
        fbz(i,j) = (fbzl-msl*bzl-bzlst*temp+bzldst*mslst)*maxs1 &
...
! divergence B flux
  ...
  subroutine flux_calc__bp(ix,jx,bxw,phiw &
                          ,bx_m,phi_m,ch)

  integer,intent(in) :: ix,jx
  real(8),intent(in) :: ch
  real(8),dimension(ix,jx,2),intent(in) :: bxw,phiw
  real(8),dimension(ix,jx),intent(out) :: bx_m,phi_m
  ...
  do j=1,jx
     do i=1,ix
        bx_m(i,j) = bxw(i,j,1) &
                     +( 0.5d0*(bxw(i,j,2)-bxw(i,j,1)) &
                       -0.5d0*(phiw(i,j,2)-phiw(i,j,1))/ch)
           
        phi_m(i,j) = phiw(i,j,1) &
                      +( 0.5d0*(phiw(i,j,2)-phiw(i,j,1)) &
                        -0.5d0*ch*(bxw(i,j,2)-bxw(i,j,1)))
     end do
  end do
  !$OMP END PARALLEL DO
  ...
  subroutine flux_calc__fbres(mdir,margin,ix,jx,fbx,curx,eta,pm &
                             ,fbx_res)

  integer,intent(in) :: ix,jx,mdir,margin
! +1 or -1, consistency between Electric fields and numerical flux
  real(8),intent(in) :: pm
  real(8),dimension(ix,jx),intent(in) :: fbx,curx,eta
  real(8),dimension(ix,jx),intent(out) :: fbx_res
...
! Flux at i+1/2
  if(mdir == 1)then
     ...
     do j=margin,jx-margin
        do i=margin,ix-margin
           fres = 0.5d0*pm*(eta(i,j)*curx(i,j) &
                 +eta(i+1,j)*curx(i+1,j))

           fbx_res(i,j) = fbx(i,j)+fres
        end do
     end do
     !$OMP END PARALLEL DO
! Flux at j+1/2
  ...
  subroutine flux_calc__feres(mdir,margin,ix,jx,fee,curx,cury,curz,bx,by,bz,eta &
                             ,fee_res)

  integer,intent(in) :: ix,jx,mdir,margin
  real(8),dimension(ix,jx),intent(in) :: fee,curx,cury,curz,bx,by,bz,eta
  real(8),dimension(ix,jx),intent(out) :: fee_res
...
! Flux at i+1/2
  if(mdir == 1)then
     ...
     do j=margin,jx-margin
        do i=margin,ix-margin
           fres = 0.5d0*(eta(i,j)*(cury(i,j)*bz(i,j)-curz(i,j)*by(i,j)) &
                 +eta(i+1,j)*(cury(i+1,j)*bz(i+1,j)-curz(i+1,j)*by(i+1,j)))
           fee_res(i,j) = fee(i,j)+fres
        end do
     end do
     !$OMP END PARALLEL DO
! Flux at j+1/2
     ...
     do j=margin,jx-margin
        do i=margin,ix-margin
           fres = 0.5d0*(eta(i,j)*(curz(i,j)*bx(i,j)-curx(i,j)*bz(i,j)) &
                 +eta(i,j+1)*(curz(i,j+1)*bx(i,j+1)-curx(i,j+1)*bz(i,j+1)))
           fee_res(i,j) = fee(i,j)+fres
        end do
     end do
     !$OMP END PARALLEL DO
