...
contains
  ...
  subroutine lr_state__1st(mdir,ix,jx,&
                           ro,pr,vx,vy,vz,bx,by,bz,phi,&
                           row,prw,vxw,vyw,vzw,bxw,byw,bzw,phiw)
  integer,intent(in) :: mdir
  integer,intent(in) :: ix,jx
  real(8),dimension(ix,jx),intent(in) :: ro,pr,vx,vy,vz,bx,by,bz,phi
  real(8),dimension(ix,jx,2),intent(out) :: row,prw,vxw,vyw,vzw,bxw,byw,bzw,phiw
  ...
  subroutine lr_state__MSCL2(mdir,ix,jx,&
                             ro,pr,vx,vy,vz,bx,by,bz,phi,ch,gm,&
                             row,prw,vxw,vyw,vzw,bxw,byw,bzw,phiw,dx,dy)

  integer,intent(in) :: ix,jx,mdir
  ...
  if(mdir == 1)then
     ...
     do j=2,jx-1
        do i=2,ix-1
           ...
           row(i-1,j,2) = qql(1)*psmv + ro1*msmv
           row(i  ,j,1) = qqr(1)*psmv + ro1*msmv
           vxw(i-1,j,2) = qql(2)*psmv + vx1*msmv
           vxw(i  ,j,1) = qqr(2)*psmv + vx1*msmv
           vyw(i-1,j,2) = qql(3)
           vyw(i  ,j,1) = qqr(3)
           vzw(i-1,j,2) = qql(4)
           vzw(i  ,j,1) = qqr(4)
           prw(i-1,j,2) = qql(5)*psmv + pr1*msmv
           prw(i  ,j,1) = qqr(5)*psmv + pr1*msmv
           bxw(i-1,j,2) = qql(6)*psmv + bx1*msmv
           bxw(i  ,j,1) = qqr(6)*psmv + bx1*msmv
           byw(i-1,j,2) = qql(7)
           byw(i  ,j,1) = qqr(7)
           bzw(i-1,j,2) = qql(8)
           bzw(i  ,j,1) = qqr(8)
           phiw(i-1,j,2) = qql(9)*psmv + phi1*msmv
           phiw(i  ,j,1) = qqr(9)*psmv + phi1*msmv
        end do
     end do
!$OMP END PARALLEL DO
  else
     ...
     do j=2,jx-1
        do i=2,ix-1
           ...
           row(i,j-1,2) = qql(1)*psmv + ro1*msmv
           row(i,j  ,1) = qqr(1)*psmv + ro1*msmv
           vxw(i,j-1,2) = qql(2)*psmv + vx1*msmv
           vxw(i,j  ,1) = qqr(2)*psmv + vx1*msmv
           vyw(i,j-1,2) = qql(3)
           vyw(i,j  ,1) = qqr(3)
           vzw(i,j-1,2) = qql(4)
           vzw(i,j  ,1) = qqr(4)
           prw(i,j-1,2) = qql(5)*psmv + pr1*msmv
           prw(i,j  ,1) = qqr(5)*psmv + pr1*msmv
           bxw(i,j-1,2) = qql(6)*psmv + bx1*msmv
           bxw(i,j  ,1) = qqr(6)*psmv + bx1*msmv
           byw(i,j-1,2) = qql(7)
           byw(i,j  ,1) = qqr(7)
           bzw(i,j-1,2) = qql(8)
           bzw(i,j  ,1) = qqr(8)
           phiw(i,j-1,2) = qql(9)*psmv + phi1*msmv
           phiw(i,j  ,1) = qqr(9)*psmv + phi1*msmv
        end do
     end do
     !$OMP END PARALLEL DO
  ...
  subroutine lr_state__MP5(mdir,ix,jx,ro,pr &
