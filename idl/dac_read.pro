...
pro dac_read3d, data, x, y, z, filename
...
pro dac_read2d, data, x, y, filename
...
for l=0L,nd-1 do begin
   mpirank=0
      for my=1,mpiy do begin
         for mx=1,mpix do begin
            print, 'Reading...',flist[l*nproc+mpirank]
            dacget2s,flist[l*nproc+mpirank],temp,margin=margin
            ixs=(mx-1)*(ix-2*margin)+1-1
            ixe=ixs+ix-1
            jxs=(my-1)*(jx-2*margin)+1-1
            jxe=jxs+jx-1
            data[ixs:ixe,jxs:jxe,l]=temp
            mpirank=mpirank+1
         endfor
      endfor
endfor

data = reform(data[margin:igx-1-margin,margin:jgx-1-margin,0:nd-1])
x = reform(x[margin:igx-1-margin])
y = reform(y[margin:jgx-1-margin])
