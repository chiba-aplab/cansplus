...
! 
subroutine getNewdt_glm_res_mpi(margin,safety,dtmin,ix,jx,kx,gm,ro,pr &
...
! INPUT
!===================
  integer,intent(in) :: ix,jx,kx,margin,mpirank
...
!  real(8) :: temp, temp2
  real(8) :: temp, temp1_1, temp1_2, temp1_3, temp2_1, temp2_2, temp2_3
...
! absolute wave velocity
           temp1_1 = (dabs(v1)+sqrt(cfxsq))/dx(i)
           temp1_2 = (dabs(v2)+sqrt(cfysq))/dy(j)
           temp1_3 = (dabs(v3)+sqrt(cfzsq))/dz(k)
           
! diffusion velocity
           temp2_1 = eta(i,j,k)/dx(i)**2
           temp2_2 = eta(i,j,k)/(dy(j))**2
           temp2_3 = eta(i,j,k)/dz(k)**2

           temp = max(temp2_3, max(temp2_2, max(temp2_1, max(temp1_3, max(temp1_1, temp1_2)))))
           if ( temp > dtmaxi) then
              dtmaxi = temp
              imin = i
              jmin = j
...
620  format('   dt = ',1pe10.3,'  < ',1pe10.3,' @ i =',i5 &
  ...
  return
end subroutine getNewdt_glm_res_mpi
