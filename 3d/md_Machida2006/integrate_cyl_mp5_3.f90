subroutine integrate_cyl_mp5_3(margin,ix,jx,kx,gm,x,dx,y,dy,z,dz,dt &
...
!
  ...
  call convert_ptoc_m(ix,jx,kx,gm,ro1,pr1,vx1,vy1,vz1,bx1,by1,bz1 &
       ,rx1,ry1,rz1,ee1)
...
! x-momentum
           srx(i,j,k) = -(ro1(i,j,k)*(vx1(i,j,k)**2-vy1(i,j,k)**2) &
...
!
  do k=3,kx-2
     do j=3,jx-2
        do i=3,ix-2
           dtodx = (2.0d0/3.0d0)*dt/dx(i)
           ro(i,j,k) = ro(i,j,k)/3.0d0 + (2.0d0/3.0d0)*ro1(i,j,k) &
                + dtodx*(frox(i-1,j,k)-frox(i,j,k))
           ee(i,j,k) = ee(i,j,k)/3.0d0 + (2.0d0/3.0d0)*ee1(i,j,k) &
                + dtodx*(feex(i-1,j,k)-feex(i,j,k))
           rx(i,j,k) = rx(i,j,k)/3.0d0 + (2.0d0/3.0d0)*rx1(i,j,k) &
                + dtodx*(frxx(i-1,j,k)-frxx(i,j,k))
           ry(i,j,k) = ry(i,j,k)/3.0d0 + (2.0d0/3.0d0)*ry1(i,j,k) &
                + dtodx*(fryx(i-1,j,k)-fryx(i,j,k))
           rz(i,j,k) = rz(i,j,k)/3.0d0 + (2.0d0/3.0d0)*rz1(i,j,k) &
                + dtodx*(frzx(i-1,j,k)-frzx(i,j,k))
           bx(i,j,k) = bx(i,j,k)/3.0d0 + (2.0d0/3.0d0)*bx1(i,j,k) &
                + dtodx*(fbxx(i-1,j,k)-fbxx(i,j,k))
           by(i,j,k) = by(i,j,k)/3.0d0 + (2.0d0/3.0d0)*by1(i,j,k) &
                + dtodx*(fbyx(i-1,j,k)-fbyx(i,j,k))
           bz(i,j,k) = bz(i,j,k)/3.0d0 + (2.0d0/3.0d0)*bz1(i,j,k) &
                + dtodx*(fbzx(i-1,j,k)-fbzx(i,j,k))
           phi(i,j,k) = phi(i,j,k)/3.0d0 + (2.0d0/3.0d0)*phi1(i,j,k) &
...
!
  ...
  do k=3,kx-2
     do j=3,jx-2
        do i=3,ix-2
           dts = (2.0d0/3.0d0)*dt
           ro(i,j,k) = ro(i,j,k)+dts*sro(i,j,k)
           rx(i,j,k) = rx(i,j,k)+dts*srx(i,j,k)
           ry(i,j,k) = ry(i,j,k)+dts*sry(i,j,k)
           rz(i,j,k) = rz(i,j,k)+dts*srz(i,j,k)
           bz(i,j,k) = bz(i,j,k)+dts*sbz(i,j,k)
           ee(i,j,k) = ee(i,j,k)+dts*see(i,j,k)
           phi(i,j,k) = phi(i,j,k)+dts*sphi(i,j,k)
           phi(i,j,k) = phi(i,j,k)*exp(-dts*ch**2/cp**2)
        end do
     end do
  end do
...
!
  ...
  return
end subroutine integrate_cyl_mp5_3
