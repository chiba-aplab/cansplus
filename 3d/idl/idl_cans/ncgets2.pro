pro ncgets2,files,narg,name,data $
   ,nx=nx,narr=narr,ndx=ndx,ix=ix,jx=jx
...
ncdf_diminq,fid,var_info.dim(2),namedim,nx
...
if (allreadflag eq 1) then begin
  ...
  if (ndx ge 1) then begin
    narr=narr(whr)
    data=fltarr(ix,jx,ndx)
    for nd=0,ndx-1 do begin
      ncdf_varget,fid,did,data0,count=[ix,jx,1],offset=[0,0,narr(nd)]
      data(*,*,nd)=data0
    endfor
  endif else begin
...
ncgets2,file,narg,name,data0,ndx=nx,ix=ix,jx=jx
...
if (ipe eq 0) then data[0:margin-1,joff:joff+jx-1-margin,*] $
   =data0[0:margin-1,margin:jx-1,*]
if (jpe eq 0) then data[ioff:ioff+ix-1-margin,0:margin-1,*] $
   =data0[margin:ix-1,0:margin-1,*]
