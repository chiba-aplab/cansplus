subroutine integrate_cyl_mp5_2nd(margin,ix,jx,kx,gm,x,dx,y,dy,z,dz,dt &
     ...
     ,eta0,vc,eta,ccx,ccy,ccz,te_factor,RadCool,time,rohalo,swtch_t,xin)
  use convert
  implicit none
...
!--Input
  ...
  real(8),intent(in) :: te_factor,RadCool,time,rohalo,swtch_t,xin
  ...
  do k=3,kx-2
     do j=3,jx-2
        do i=3,ix-2
           ...
           ee(i,j,k) = ee(i,j,k)+dtody*(feeyr(i,j-1,k)-feeyr(i,j,k))
           ...
           bx(i,j,k) = bx(i,j,k)+dtody*(fbxyr(i,j-1,k)-fbxyr(i,j,k))
           bz(i,j,k) = bz(i,j,k)+dtody*(fbzyr(i,j-1,k)-fbzyr(i,j,k))
  ...
  do k=3,kx-2
     do j=3,jx-2
        do i=3,ix-2
           ...
           ee(i,j,k) = ee(i,j,k)+dtodz*(feezr(i,j,k-1)-feezr(i,j,k))
           ...
           bx(i,j,k) = bx(i,j,k)+dtodz*(fbxzr(i,j,k-1)-fbxzr(i,j,k))
           by(i,j,k) = by(i,j,k)+dtodz*(fbyzr(i,j,k-1)-fbyzr(i,j,k))
