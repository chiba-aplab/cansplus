subroutine integrate_cyl_mp5_1st_telimit(margin,ix,jx,kx,gm,x,dx,y,dy,z,dz,dt &
     ,gx,gz,floor,ro,pr,vx,vy,vz,bx,by,bz,phi,ch,cr &
     ,ro1,pr1,vx1,vy1,vz1,bx1,by1,bz1,phi1 &
     ,eta0,vc,ccx,ccy,ccz,RadCool,te_factor &
     ,time,rohalo,swtch_t,te_limit)
  use convert
  use get_temperature
  implicit none
...
!--Input
  ...
  real(8),intent(in) :: RadCool,te_factor,time,rohalo,swtch_t,te_limit
...
!
  ...
  call get_temperature(ix,jx,kx,ro,pr,te,te_factor)
	...
	! energy
		   ...
		   if (time .gt. swtch_t) then
			  if (ro(i,j,k) .gt. rohalo) then
		        see = see - RadCool*(ro(i,j,k)**2)*sqrt(te(i,j,k))
			  endif
           endif
...
!
  ...
  return
end subroutine integrate_cyl_mp5_1st_telimit
