...
! 
subroutine getNewdt_glmcyl_res_mpi(margin,safety,dtmin,ix,jx,kx,gm,ro,pr &
     ,vx,vy,vz,bx,by,bz,x,dx,y,dy,z,dz,eta &
     ,dt,merr,ch,mpirank,xin)
...
!                + (dabs(v3)+sqrt(cfzsq))/dz(k)
			...
			dti1 = (dabs(v1)+sqrt(cfxsq))/dx(i)
			dti2 = (dabs(v2)+sqrt(cfysq))/(x(i)*dy(j))
			dti3 = (dabs(v3)+sqrt(cfzsq))/z(k)
...
620  format('   dt = ',1pe10.3,'  < ',1pe10.3,' @ i =',i5 &
  ...
  return
end subroutine getNewdt_glmcyl_res_mpi
