subroutine hancock_diff_e(mdir,ix,jx,kx,gm,ch,ro,pr,vx,vy,vz,bx,by,bz,phi &
     ,roHDiff,prHDiff,vxHDiff,vyHDiff,vzHDiff,bxHDiff,byHDiff,bzHDiff,phiHDiff)
  implicit none
...
! step 1 cal.eigen Matrix
           call getEigenMatric(nWave,gm,ch,ro(i,j,k),vx(i,j,k),vy(i,j,k),vz(i,j,k) &
...
!----
! step 1 cal.eigen Matrix
           call getEigenMatric(nWave,gm,ch,ro(i,j,k),vy(i,j,k),vz(i,j,k),vx(i,j,k) &
...
!----
! step 1 cal.eigen Matrix
           call getEigenMatric(nWave,gm,ch,ro(i,j,k),vz(i,j,k),vx(i,j,k),vy(i,j,k) &
...
! step 4 convert primitive
  ...
  return
contains
  subroutine getEigenMatric(nWave,gm,ch,ro,vx,vy,vz,pr,bx,by,bz,phi,rem,lem)
    implicit none
    ...
    cf2 = 0.5d0*(temp_sum+sqrt(temp_diff**2+cf2_cs2)
    cs2 = 0.5d0*(temp_sum-sqrt(temp_diff**2+cf2_cs2)
    ...
    if(cf2_cs2 == 0.0d0)then
       ...
       alf = sqrt((aa2-cs2)/(cf2-cs2))
       als = sqrt((cf2-aa2)/(cf2-cs2))
    endif
...
! Normalized factor
    ...
    rem(3,7) = betaz
    rem(4,7) = -betay
    rem(7,7) = -betaz*bxsign*sqrtro
    rem(8,7) = betay*bxsign*sqrtro
    ...
    lem(1,5) = nnff*alf/ro
    lem(1,7) = nnff*als*betay/ro
    lem(1,8) = nnff*als*betaz/ro
    ...
    lem(3,5) = nnss*alf/ro
    lem(3,7) = -nnss*alf*betay/ro
    lem(3,8) = -nnss*alf*betaz/ro

    lem(4,1) = 1.0d0
    lem(4,5) = -1.0d0/aa2

    lem(5,2) = nnss*ccss
    lem(5,3) = nnss*qqf*betay
    lem(5,4) = nnss*qqf*betaz
    lem(5,5) = nnss*als/ro
    lem(5,7) = -nnss*alf*betay/ro
    lem(5,8) = -nnss*alf*betaz/ro
    ...
    lem(7,3) = 0.5d0*betaz
    lem(7,4) = -0.5d0*betay
    lem(7,7) = 0.5d0*betaz*bxsign/sqrtro
    lem(7,8) = 0.5d0*betay*bxsign/sqrtro

    lem(8,2) = nnff*ccff
    lem(8,3) = -nnff*qqs*betay
    lem(8,4) = -nnff*qqs*betaz
    lem(8,5) = nnff*alf/ro
    lem(8,7) = nnff*als*betay/sqrtro
    lem(8,8) = nnff*als*betaz/sqrtro
