...
!!$             +bzrdst*(msrst-msm)+bzrst*(msr-msrst)) 
           ...
           if (slst >= 0.0d0)then
              ...
              temp1 = srst - sm
              temp2 = sm - slst
              fro(i,j,k) = fror - sr*ror + temp*rorst + temp1*rordst + temp2*roldst + slst*rolst
              frx(i,j,k) = frxr - sr*rxr + temp*rxrst + temp1*rxrdst + temp2*rxldst + slst*rxlst
              fry(i,j,k) = fryr - sr*ryr + temp*ryrst + temp1*ryrdst + temp2*ryldst + slst*rylst
              frz(i,j,k) = frzr - sr*rzr + temp*rzrst + temp1*rzrdst + temp2*rzldst + slst*rzlst
              fee(i,j,k) = feer - sr*eer + temp*eerst + temp1*eerdst + temp2*eeldst + slst*eelst
              fby(i,j,k) = fbyr - sr*byr + temp*byrst + temp1*byrdst + temp2*byldst + slst*bylst
              fbz(i,j,k) = fbzr - sr*bzr + temp*bzrst + temp1*bzrdst + temp2*bzldst + slst*bzlst
